...
-- Show all customers and their bookings (even if they have no bookings)
...
-- IN
-- Speed up searching bookings by date
CREATE INDEX idx_booking_date ON Bookings(booking_date);

-- Speed up searching showtimes by date
CREATE INDEX idx_show_date ON Showtimes(show_date);


-- Speed up searching movies by title or genre
CREATE FULLTEXT INDEX idx_movie_title ON Movies(title);
CREATE FULLTEXT INDEX idx_movie_genre ON Movies(genre);

 
EXPLAIN
SELECT b.booking_id, c.fName, c.lName
FROM Bookings b
INNER JOIN Customers c 
